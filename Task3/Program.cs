...
namespace Task3
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            try
            {
                long FolderSizeIn = GetFolderSize(FolderPath);
                Console.WriteLine($"Размер папки {FolderPath}: {FolderSizeIn} байт!");

                CleanFolder(FolderPath);
                Console.WriteLine("Очистка папки от файлов завершена!");

                long FolderSizeOut = GetFolderSize(FolderPath);
                Console.WriteLine($"Размер папки после очистки {FolderPath}: {FolderSizeOut} байт!");
            }
